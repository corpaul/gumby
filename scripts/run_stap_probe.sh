...
if [ -z "$PROBE_NAME" ]; then
	PROBE_NAME="log_io_writes.stp"
fi

$STAPPATH/stap "$SCRIPTDIR"/stp/$PROBE_NAME -I $VIRTUALENV_DIR/tapsets/ -DMAXMAPENTRIES=50000 -DSTP_NO_OVERLOAD -DMAXSTRINGLEN=4096 -DTRYLOCKDELAY=300 -DMAXSKIPPED=10000 -DMAXACTION=1000 -o $2 -c "env 'LD_LIBRARY_PATH=$LD_LIBRARY_PATH' $TEST_COMMAND"
